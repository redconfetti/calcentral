describe User::Academics::Enrollment::Terms do
  let(:user_uid) { '61889' }
  let(:user_student_attributes) { double(:student_attributes)  }
  let(:user_enrollment_term_instructions) { double(:enrollment_term_instructions) }
  let(:user_term_plans) { [double(:user_term_plan)] }
  let(:user) do
    double(
      :user,
      :uid => user_uid,
      student_attributes: user_student_attributes,
      enrollment_term_instructions: user_enrollment_term_instructions,
      term_plans: user_term_plans,
    )
  end
