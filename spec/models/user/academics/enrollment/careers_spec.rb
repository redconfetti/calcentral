describe User::Academics::Enrollment::Careers do
  ...
  before do
    allow(::User::Academics::Enrollment::Career).to receive(:new) do |data|
      double(:enrollment_career, :career_code => data[:acadCareer])
    end
  end
  ...
  describe '#all' do
    ...
    it 'returns array of all enrollment career objects' do
      ...
      expect(all_careers[0].career_code).to eq 'UGRD'
      expect(all_careers[1].career_code).to eq 'GRAD'
    end
  end

  describe '#find_by_career_code' do
    let(:career_code) { 'UGRD' }
    let(:found_career) { subject.find_by_career_code(career_code) }
    it 'returns single career matching specified career code' do
      expect(found_career.career_code).to eq 'UGRD'
    end
  end
end
