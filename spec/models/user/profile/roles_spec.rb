describe User::Profile::Roles do
  ...
  describe '.roles_array' do
    ...
    let(:result) { described_class.roles_array(legacy_roles_hash) }
  ...
  describe '.roles_hash' do
    let(:result) { described_class.roles_hash }
  ...
  describe '#is_unreleased_applicant?' do
    ...
    context 'when user is not an applicant' do
      ...
      it 'returns false' do
        expect(subject.is_unreleased_applicant?).to eq false
      end
    end
    context 'when user is an applicant' do
      ...
      context 'when user has no other roles other than presir' do
        let(:is_student?) { false }
        it 'returns true' do
          expect(subject.is_unreleased_applicant?).to eq true
        end
      end
    end
